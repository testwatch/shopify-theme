/* Simple jQuery Equal Heights @version 1.5.1. */
...
timber.accessibleNav = function() {
  ...
  function hideDropdown($el) {
    ...
    $dropdowns.removeClass('nav-hover');
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    initProductVariant: function() {
      ...
      var selectCallback = function(variant, selector) {
        ...
        const setGiftPrice = () => {
  let giftPrice = document.querySelector('.single-priduct-gift-price .money');
  if (giftPrice) {
    giftPrice.textContent = variant.title;
    console.log(variant.title);
  } else {
    console.log('giftPrice element not found');
  }
}
...
function formatPrice(priceInCents) {
  ...
  return '$' + price.toFixed(2);  // Adjust this formatting as needed
    ...
    $('#AddToCart #AddToCartText').html('Add to Cart');
     ...
     $('.cart_price_btn button .money').css('display' , 'inline-block');
          console.log('in cart');
          // Update text for each .money element within .cart_price_btn buttons
  $('.cart_price_btn button .money').each(function() {
    var priceInCents = parseFloat($(this).attr('data-money')) || 0;
    ...
    $(this).html(formattedPrice);
          ...
          // Update text for each element with class "money"
          for (var i = 0; i < moneyElements.length; i++) {
            moneyElements[i].innerText = formattedPrice;
          }*/
} else {
            $('#AddToCart #AddToCartText').html('Sold Out');
          console.log('else sold out')
            $('.cart_price_btn').addClass('sold_out_btn');
            //$('.cart_price_btn button .money').css('display' , 'none');
          $('.cart_price_btn button .money').each(function() {
    var priceInCents = parseFloat($(this).attr('data-money')) || 0;
    var formattedPrice = Shopify.formatMoney(variant.price);
    $(this).html(formattedPrice);
            ...
            $('#AddToCart').css('opacity', '0.6');
            $('#AddToCart').css('background', 'red');
}

        
        $("#varient-sku-container").text(variant.sku)
         // EXTEND -- Set active Extend product on variant change	
        window.dispatchEvent(new CustomEvent('setVariant', {detail: {variant: variant.id}}))	
        // EXTEND -- End Extend code
        console.log(variant);

        
let elm = document.querySelector('.grid-product__on-sale .money');
let elmCont = document.querySelector('.grid-product__on-sale');
let perElm = document.querySelector('.grid-product__on-sale .percentage');
...
if (elmCont) {
  if (variant.compare_at_price !== null) {
    if (variant.compare_at_price === 0) {
      ...
      let price = (variant.compare_at_price - variant.price) / 100;
      let value = ((variant.compare_at_price - variant.price) / variant.compare_at_price) * 100;
      let percentage = Math.round(value);
      perElm.innerHTML = percentage;
...
$('.spec_header h3').text(variant.sku);
  var id = variant.id;
$('.product-info-name').each(function() {
  if($(this).hasClass(id)){
...
$('.product-info-value').each(function() {
  if($(this).hasClass(id)){
    $(this).addClass("active-item");
  }else{
    $(this).removeClass("active-item");
...
//             $('#AddToCart').css('background', 'red');
        ...
        // END SWATCHES
        // $('.spec_header span, .free-space_new li span , .free-space_second li span, .free-space_third li span,.extra_product_outer_list,.custom_variant_title').each(function () {
        //   console.log(variant.id);
        //   if($(this).hasClass(variant.id)) {
        //     $(this).addClass('active_title');
        //     $(this).removeClass('disabled_title');
        //   } else{
        //     $('.spec_header span, .free-space_new li span, .free-space_second li span, .free-space_third li span,.extra_product_outer_list,.custom_variant_title').addClass('disabled_title');
        //     $(this).removeClass('active_title');
        //   }    
        // });
        ...
        $('.gallery-video #variant_id').each(function(e){
          if ($(this).hasClass(variant.id)) {
            $(this).addClass('active_title');
            // $(".video-wrapper iframe")[0].src += "&autoplay=1";

            $(document).on("click",".video-item",function(ev) {  
              if($(".video_cls.video_wrap iframe").length > 0) {
                $(".video_cls.video_wrap iframe")[0].src += "&autoplay=1";
              }
            });
          } else{
            $(this).removeClass('active_title');
        ...
        // Fix for TypeError: affirm.ui.refresh is not a function. (In 'affirm.ui.refresh()', 'affirm.ui.refresh' is undefined
        // typeof affirm == 'object' && affirm.ui.refresh();
...
//Start Clyde Script
  (typeof Clyde !='undefined') && Clyde.init({key: "ck_live_CzaRXSsB7pX41B4y"},
    function cb() {
      ...
      if(Clyde.getSettings().productPage == true){
        Clyde.setActiveProduct(productData,function(){
        Clyde.appendToSelector("#yourClydePDPSelector", 'list');
        });
        ...
        item.addEventListener("click", function () {
         ...
         if (Clyde.checkInit() && Clyde.checkReady()) {
          ...
          if (selectedContract) {Clyde.loadContractsForProduct(productData,pdpAdd);}
         }
         },500);
        });
        })
      ...
      if(Clyde.getSettings().modal == true){
        ...
        item.addEventListener("click", function () {
         ...
         if (Clyde.checkInit() && Clyde.checkReady()) {
          let selectedContract = Clyde.getSelectedContract(); 
          if (!selectedContract) {Clyde.showModal(null, closeModalCb);}
         }
         },500);
        });
        })
  ...
  function closeModalCb() {
    ...
    if (selectedContract) {addClydeToCart(selectedContract);} 
  }
      
  async function pdpAdd(){
    let selectedContract = Clyde.getSelectedContract();
    if (selectedContract) {addClydeToCart(selectedContract);}   
  }
         
  function loadSlideClydeCartCTAs(productsInCartArray) {
    productsInCartArray.forEach(function (sku) {
    Clyde.appendToSelectorWithSku(sku, `#clyde--${cartItems[sku].key}`, closeModalCb);
    });
  } 
  
  function addClydeToCart(selectedContract){
    let productName;
    let activeVariantId = Clyde.getActiveProduct().sku;
    ...
    productPage: function(options) {
      ...
      if (variant) {
        ...
        if (variant.compare_at_price > variant.price) {
          ...
          $('.template-product .price_part.desk-hidden #ComparePrice').html(Shopify.formatMoney(variant.compare_at_price, moneyFormat)).removeClass(this.classes.hide);
          ...
          $('.template-product .price_part.desk-hidden #ComparePrice').addClass(this.classes.hide).html('');
    ...
    $.get(recommendationsSectionUrl).then(
      function(section) {
        ...
        if (recommendationsMarkup.trim() !== '') {
          ...
          document.dispatchEvent(new CustomEvent(':productRecommendations'));
        }
      }.bind(this)
    );
  }
...
$(window).scroll(function() {    
  ...
  if (scroll >= 600) {
    ...
    $(".header1").addClass("darkHeader");
    $('.gf-sort-wrap').addClass('fixed-filter');
  } else {
    $(".header1").removeClass("darkHeader");
    $('.gf-sort-wrap').removeClass('fixed-filter');
  }
});
  ...
  $('.gfqv-swatch-value').click(function(){
    $('.gfqv-swatch-value').removeClass('is-selected')
    $(this).addClass('is-selected');
    var data_image=$(this).find('.gfqv-product__swatch-inner').attr('data_image');
    $('.gfqv-product-gallery .gfqv-product-image img').attr('src',data_image);
    $('.gfqv-product-gallery .gfqv-product-image img').attr('data-src',data_image);
    var variant_id=$(this).find('.gfqv-product__swatch-inner').attr('data_variant_id');
    $('.product_page_cta a').attr('data_variant_id',variant_id);
    var data_values =$(this).find('.gfqv-product__swatch-inner').attr('data_value');
    $('.gfqv-swatchs .gfqv-current-value').html(data_values);
  });
$('.product_page_cta a').on("click",function(e){
    e.preventDefault();
var data_id=$(this).attr('data_variant_id');
     $.ajax({
      type: 'POST', 
      url: '/cart/add.js',
      dataType: 'json', 
...
$(".gfqv-swatch-values .main-product-id").each(function(){
  var variant_id = $(this).attr("data_variant_id");
  var variant_match_id = $(this).attr("data_match_id");
if(variant_id == variant_match_id)
{
$(this).parents('.gfqv-swatch-value').addClass('is-selected');
}
});

  $(document).mouseup(function (e) {
    if ($(e.target).closest(".gfqv-container").length === 0) {
      $('.gfqv-modal').hide();
    }
  });
...
$('.mobile-banner-image-search').click(function() {
  $('div#announcement-slider').css('display', 'none');
  $('.section-header.section-header--large.mobile-search-bar-popups').css('transform', 'translate(0)');
  $("#shopify-section-header .header1 .header-bar").css("padding', '0px !important");  
  $(".input-type-typ").click();
  $("main.wrapper.main-content").addClass("overlaymobilesearch");
  $("input.input-group-field.input-type-typ").focus();
});

$('.close-icon-mobile-search').click(function() {
  $('div#announcement-slider').css('display', 'block');
  $('.section-header.section-header--large.mobile-search-bar-popups').css('transform', 'translateY(-186%)');
  $("main.wrapper.main-content").removeClass("overlaymobilesearch");
});


// if ($(window).width() < 767) {
//   $('.grid-collection-fac').slick({
//       slidesToShow: 1.5,
//     slidesToScroll: 1,
//       arrows: false,
//       dots: false,
